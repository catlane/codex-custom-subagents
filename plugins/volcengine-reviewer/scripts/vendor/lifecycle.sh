...
validate_agent_file() {
  [ -e "$1" ] || return 0
  begin="# BEGIN custom-subagents managed agent id=$2 plugin=$3"
  end="# END custom-subagents managed agent id=$2 plugin=$3"
  ...
  if [ -f "$4" ]; then
    expected_agent=$(jxa render-agent-spec-state "$CANONICAL_AGENT_SPEC" "$4" "$2" "$5" "codex-custom-subagent/$2")
    actual_agent=$(cat "$1")
...
has_managed_residue() {
  target_agent=$1; generated_catalog=$2; base_catalog=$3; workflow=$4; config=$5
  { [ -e "$target_agent" ] || [ -L "$target_agent" ]; } && return 0
...
install() {
  [ "$#" = 5 ] || die "install requires agent ID, role, provider ID, endpoint, and model"
  agent_id=$1; role=$2; provider=$3; endpoint=$4; model=$5
  jxa validate-agent-input "$agent_id" "$role" "$provider" "$endpoint" "$model" >/dev/null
  ...
  [ "$plugin_name" = "$agent_id" ] || die "agent ID does not match plugin ownership manifest"
  ...
  workflow_file="$SUBAGENT_HOME/AGENTS.md"; agent_name=$(printf '%s' "$agent_id" | tr '-' '_'); agent_file="$SUBAGENT_HOME/agents/$agent_name.toml"
  initial_agents_shape=$(agents_shape "$workflow_file")
  validate_markers "$workflow_file"; validate_agent_file "$agent_file" "$agent_id" "$plugin_name" "$state_file" "$catalog_file"
  assert_safe_target "$state_file"; assert_safe_target "$catalog_file"; assert_safe_target "$base_catalog_file"
  assert_safe_target "$config_file"; assert_safe_target "$workflow_file"; assert_safe_target "$agent_file"
  ...
  managed_artifact=0
  [ -e "$agent_file" ] && managed_artifact=1
  ...
  if [ "$managed_artifact" = 1 ] || [ -f "$state_file" ]; then
    ...
    target_registered=$(jxa agent-present "$state_file" "$agent_id")
    if [ -e "$agent_file" ] || [ "$target_registered" = 1 ]; then
      jxa validate-lifecycle-state "$state_file" "$catalog_file" "$base_catalog_file" "$agent_id" >/dev/null
    else
  ...
  keychain_service="codex-custom-subagent/$agent_id"
  rendered_agent=$(jxa render-agent-spec-args "$CANONICAL_AGENT_SPEC" "$agent_id" "$provider" "$endpoint" "$model" "$catalog_file" "$keychain_service")
  ...
  agent_json=$(printf '{"id":"%s","role":"%s","provider":"%s","endpoint":"%s","model":"%s"}' "$agent_id" "$role" "$provider" "$endpoint" "$model")
  if [ "$fresh_install" = 1 ]; then atomic_write "$base_catalog_file" "$base_catalog_source"; fi
  jxa install-state "$state_file" "$agent_json" "$catalog_file" "$base_catalog_file" "$base_catalog_source" \
  ...
  { printf '%s\n' "# BEGIN custom-subagents managed agent id=$agent_id plugin=$plugin_name"; printf '%s\n' "$rendered_agent"; printf '%s\n' "# END custom-subagents managed agent id=$agent_id plugin=$plugin_name"; } >"$temp_dir/agent.toml"
  atomic_write "$agent_file" "$temp_dir/agent.toml"
...
uninstall() {
  [ "$#" = 1 ] || die "uninstall requires an agent ID"
  agent_id=$1; jxa validate-agent-id "$agent_id" >/dev/null
  ...
  agent_name=$(printf '%s' "$agent_id" | tr '-' '_'); agent_file="$SUBAGENT_HOME/agents/$agent_name.toml"
  plugin_name=$(jxa manifest-name "$PLUGIN_ROOT/.codex-plugin/plugin.json")
  [ "$plugin_name" = "$agent_id" ] || die "agent ID does not match plugin ownership manifest"
  assert_safe_target "$state_file"; assert_safe_target "$catalog_file"; assert_safe_target "$base_catalog_file"
  assert_safe_target "$workflow_file"; assert_safe_target "$agent_file"
  assert_safe_target "$SUBAGENT_HOME/config.toml"; assert_safe_target "$state_dir/backups/.custom-subagents-preflight"
  if [ ! -f "$state_file" ]; then
    has_managed_residue "$agent_file" "$catalog_file" "$base_catalog_file" "$workflow_file" "$SUBAGENT_HOME/config.toml" || return 0
  ...
  target_registered=$(jxa agent-present "$state_file" "$agent_id")
  target_file_present=0
  [ -e "$agent_file" ] && target_file_present=1
  if [ "$target_registered" = 0 ] && [ "$target_file_present" = 0 ]; then
    return 0
  fi
  [ "$target_registered" = 1 ] && [ "$target_file_present" = 1 ] ||
    die "target agent file and lifecycle state disagree; restore from backup or reconfigure custom subagents"
  validate_with_recovery validate_agent_file "$agent_file" "$agent_id" "$plugin_name" "$state_file" "$catalog_file"
  ...
  trap 'cleanup_and_rollback 130' INT
  trap 'cleanup_and_rollback 143' TERM
  jxa uninstall-state "$state_file" "$agent_id" >"$temp_dir/state.json"
  if grep -F '"agents": []' "$temp_dir/state.json" >/dev/null; then
    ...
    jxa catalog "$base_catalog_file" "$primary_model" >"$temp_dir/catalog.json"; atomic_write "$catalog_file" "$temp_dir/catalog.json"
    jxa workflow "$temp_dir/state.json" >"$temp_dir/workflow.md"; render_workflow_file "$workflow_file" "$temp_dir/workflow.md" "$temp_dir/AGENTS.md"; atomic_write "$workflow_file" "$temp_dir/AGENTS.md"
    remove_target "$agent_file"
  fi
  rm -rf "$temp_dir"; COMMITTED=1; trap - EXIT HUP INT TERM; release_lock
}
validate_registration() {
  [ "$#" = 1 ] || die "validate-registration requires an agent ID"
  agent_id=$1; jxa validate-agent-id "$agent_id" >/dev/null
  state_dir="$SUBAGENT_HOME/custom-subagents"; state_file="$state_dir/state.json"
  catalog_file="$state_dir/models-v1.json"; base_catalog_file="$state_dir/base-model-catalog.json"
  ...
  [ "$plugin_name" = "$agent_id" ] || die "agent ID does not match plugin ownership manifest"
  assert_safe_target "$state_file"; assert_safe_target "$catalog_file"; assert_safe_target "$base_catalog_file"
  assert_safe_target "$workflow_file"; assert_safe_target "$config_file"; assert_safe_target "$agent_file"
  ...
  jxa validate-catalog-matches-state "$state_file" "$base_catalog_file" "$catalog_file" >/dev/null
  validate_workflow_matches_state "$workflow_file" "$state_file"
  [ -f "$agent_file" ] && [ ! -L "$agent_file" ] || die "managed agent file is missing or unsafe"
  validate_agent_file "$agent_file" "$agent_id" "$plugin_name" "$state_file" "$catalog_file"
}
