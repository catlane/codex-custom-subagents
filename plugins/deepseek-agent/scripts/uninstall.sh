...
cleanup_removed_credential_on_signal() {
  ...
  if [ -f "$signal_state" ] && [ ! -L "$signal_state" ]; then
    ...
    signal_registration=$(/usr/bin/osascript -l JavaScript "$VENDOR_DIR/state.js" \
      provider-present "$signal_state" deepseek-agent 2>/dev/null)
  ...
  case "$signal_keychain_status" in
    ...
    44) return 0 ;;
    *)
      printf '%s\n' "deepseek-agent: interrupted uninstall could not inspect Keychain status=$signal_keychain_status service=codex-custom-subagent/deepseek-agent account=api-key; retry uninstall cleanup." >&2
...
/bin/sh "$VENDOR_DIR/lifecycle.sh" uninstall deepseek-agent
...
case "$keychain_lookup_status" in
  0) keychain_delete deepseek-agent || die 'Keychain cleanup failed after lifecycle cleanup; retry uninstall after resolving Keychain access' ;;
  44) ;;
  *)
    printf '%s\n' "deepseek-agent: Keychain lookup failed status=$keychain_lookup_status service=codex-custom-subagent/deepseek-agent account=api-key; resolve Keychain access and retry uninstall." >&2
    exit "$keychain_lookup_status"
    ;;
esac
printf '%s\n' 'DeepSeek provider lifecycle, profiles, and Keychain item removed.'
