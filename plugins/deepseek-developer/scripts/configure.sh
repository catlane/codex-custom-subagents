...
/usr/bin/osascript -l JavaScript "$VENDOR_DIR/state.js" validate-agent-input \
  deepseek-developer development deepseek "$ENDPOINT" "$MODEL" >/dev/null
...
credential_binding_is_committed() {
  ...
  committed_binding=$(/usr/bin/osascript -l JavaScript "$VENDOR_DIR/state.js" \
    keychain-binding-status "$binding_state" deepseek-developer "$ENDPOINT" 2>/dev/null)
  ...
  /bin/sh "$VENDOR_DIR/lifecycle.sh" validate-registration deepseek-developer >/dev/null 2>&1
...
cleanup_fresh_credential_on_signal() {
  ...
  case "$signal_keychain_status" in
    ...
    44) return 0 ;;
    *)
      printf '%s\n' "deepseek-developer: interrupted configuration could not inspect Keychain status=$signal_keychain_status service=codex-custom-subagent/deepseek-developer account=api-key; retry uninstall cleanup." >&2
...
run_lifecycle() {
  ...
  CUSTOM_SUBAGENT_PRODUCTION_MODE="$RUNTIME_LIFECYCLE_PRODUCTION_MODE" \
  CUSTOM_SUBAGENT_PRODUCTION_APPROVAL="$RUNTIME_LIFECYCLE_PRODUCTION_APPROVAL" \
  /bin/sh "$VENDOR_DIR/lifecycle.sh" install deepseek-developer development deepseek "$ENDPOINT" "$MODEL"
}
...
credential_binding_status() {
  ...
  [ ! -L "$binding_state" ] && [ -f "$binding_state" ] ||
    die 'credential binding state is not a regular managed file service=codex-custom-subagent/deepseek-developer account=api-key; restore lifecycle state before retrying'

  set +e
  binding_result=$(/usr/bin/osascript -l JavaScript "$VENDOR_DIR/state.js" \
    keychain-binding-status "$binding_state" deepseek-developer "$ENDPOINT")
  binding_result_status=$?
  set -e
  [ "$binding_result_status" = 0 ] ||
    die 'credential binding state validation failed service=codex-custom-subagent/deepseek-developer account=api-key; restore lifecycle state before retrying'
...
case "$keychain_lookup_status" in
  0)
    ...
    case "$binding_status" in
      match) run_lifecycle ;;
      mismatch|absent)
        die 'existing credential endpoint binding is mismatched or unknown service=codex-custom-subagent/deepseek-developer account=api-key; first run the deepseek-developer uninstall cleanup, then configure again and enter a fresh hidden-dialog key'
        ;;
    esac
    ;;
  44)
    ...
    set +e
    keychain_prompt_store deepseek-developer
    prompt_status=$?
    set -e
    [ "$prompt_status" = 0 ] || {
    ...
    if [ "$lifecycle_status" -ne 0 ]; then
      ...
      if ! keychain_delete deepseek-developer; then
        printf '%s\n' 'deepseek-developer: lifecycle failed and Keychain cleanup incomplete service=codex-custom-subagent/deepseek-developer account=api-key; remove that exact item after resolving Keychain access, then rerun cleanup.' >&2
        exit 1
      fi
    ...
    ;;
  *)
    printf '%s\n' "deepseek-developer: Keychain lookup failed status=$keychain_lookup_status service=codex-custom-subagent/deepseek-developer account=api-key; resolve Keychain access and retry." >&2
    exit "$keychain_lookup_status"
    ;;
esac

printf '%s\n' 'DeepSeek developer configured. Restart Codex and begin a fresh task.'
