...
the native hidden dialog. Elevated (unsandboxed) execution with an
interactive terminal attached is mandatory for `scripts/configure.sh`: request
the user's approval for that elevation and run the script only with it,
because a sandboxed agent shell cannot present the native hidden dialog or the
native model chooser, and without an interactive terminal even the hidden
fallback prompts cannot be answered. If the user declines elevation, stop: do
not configure, do not attempt any fallback, and never substitute hand-written
agent files, environment variables, or launcher scripts for this managed flow.
The only remaining path is to give the user the exact configure command to run
themselves in an interactive Terminal window.
