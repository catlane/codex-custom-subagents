...
cleanup_fresh_credential() {
  ...
  case "$cleanup_keychain_status" in
    ...
    44) return 0 ;;
    *)
      printf '%s\n' "volcengine-agent: cleanup could not inspect Keychain status=$cleanup_keychain_status service=codex-custom-subagent/volcengine-agent account=api-key; remove that exact item after resolving Keychain access." >&2
      return 1
      ;;
  esac
}
cleanup_fresh_credential_on_signal() {
  cleanup_fresh_credential || {
    printf '%s\n' 'volcengine-agent: interrupted configuration cleanup incomplete; retry uninstall cleanup.' >&2
    return 1
  }
}
...
if [ -z "$MODEL" ]; then
  set +e
  MODEL=$(model_discovery_resolve "$ENDPOINT" volcengine-agent "$MODEL_DISCOVERY_PARSER" "$MODEL_DISCOVERY_CHOICE_SCRIPT")
  ...
  case "$model_discovery_status" in
    ...
    2)
      ...
      ;;
    *)
      printf '%s\n' 'volcengine-agent: model list could not be loaded; enter the model or deployment ID manually.' >&2
      set +e
      ...
      case "$manual_model_status" in
        ...
        *)
          cleanup_fresh_credential || true
          printf '%s\n' 'volcengine-agent: automatic model discovery and manual model input are unavailable; rerun with --model MODEL_OR_DEPLOYMENT_ID in an interactive Terminal.' >&2
          exit 78
          ;;
      esac
      ;;
