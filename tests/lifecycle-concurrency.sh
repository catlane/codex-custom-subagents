...
FIRST_PLUGIN="$TEMP_ROOT/deepseek-developer"
...
cp "$ROOT/plugins/deepseek-developer/.codex-plugin/plugin.json" "$FIRST_PLUGIN/.codex-plugin/plugin.json"
...
assert_contains "$TEST_HOME/custom-subagents/state.json" '"id": "reviewer-agent"'
...
set +e
run_lifecycle "$SECOND_PLUGIN" \
  install reviewer-agent review volcengine http://localhost:11434 '' \
...
set +e
run_lifecycle "$SECOND_PLUGIN" uninstall reviewer-agent \
