...
[ -x "$CONFIGURE" ] || fail 'DeepSeek configure entrypoint is not executable'
[ -x "$UNINSTALL" ] || fail 'DeepSeek uninstall entrypoint is not executable'
...
SENTINEL='SECRET_MUST_NOT_APPEAR_TASK4_8D92'
...
run_configure() {
  ...
  sh "$CONFIGURE" --endpoint "${1:-https://api.deepseek.com}" --model "${2:-deepseek-chat}"
}
...
FAKE_DIALOG_VALUE=forged-secret sh "$FORGED_PLUGIN/scripts/configure.sh" --model deepseek-chat \
...
assert_configure_gate_rejects() {
  ...
  env CODEX_HOME="$gate_home" FAKE_SECURITY_STATE="$gate_state" FAKE_SECURITY_LOG="$gate_log" \
    ...
    "$@" sh "$CONFIGURE" --model deepseek-chat \
...
set +e
run_configure https://different.deepseek.example/v1 deepseek-reasoner >"$CAPTURED/endpoint-change.out" 2>"$CAPTURED/endpoint-change.err"
endpoint_change_status=$?
set -e
[ "$endpoint_change_status" -ne 0 ] || fail 'existing DeepSeek credential was rebound to a different endpoint'
...
assert_not_contains "$CAPTURED/endpoint-change.err" 'https://api.deepseek.com'
assert_not_contains "$CAPTURED/endpoint-change.err" 'https://different.deepseek.example/v1'
...
FAKE_DIALOG_SCRIPT_LOG="$FAKE_DIALOG_SCRIPT_LOG" \
sh "$CONFIGURE" --model deepseek-chat >"$CAPTURED/unknown-binding.out" 2>"$CAPTURED/unknown-binding.err"
unknown_binding_status=$?
set -e
[ "$unknown_binding_status" -ne 0 ] || fail 'unregistered DeepSeek credential was bound to an endpoint'
...
FAKE_DIALOG_SCRIPT_LOG="$FAKE_DIALOG_SCRIPT_LOG" \
sh "$CONFIGURE" --model deepseek-chat >"$CAPTURED/malformed-binding.out" 2>"$CAPTURED/malformed-binding.err"
malformed_binding_status=$?
set -e
[ "$malformed_binding_status" -ne 0 ] || fail 'malformed state allowed existing DeepSeek credential reuse'
...
# A locked or denied Keychain lookup stops before dialog, store, or lifecycle mutation.
...
sh "$CONFIGURE" --model deepseek-chat >"$CAPTURED/locked.out" 2>"$CAPTURED/locked.err"
locked_status=$?
set -e
...
# An accepted empty dialog response cannot create a Keychain item or lifecycle state.
...
: >"$EMPTY_STATE"
: >"$EMPTY_LOG"
empty_before=$(cksum "$EMPTY_HOME/config.toml")
set +e
...
sh "$CONFIGURE" --model deepseek-chat >"$CAPTURED/empty.out" 2>"$CAPTURED/empty.err"
empty_status=$?
set -e
[ "$empty_status" -ne 0 ] || fail 'empty dialog response unexpectedly configured the plugin'
assert_equals "$empty_before" "$(cksum "$EMPTY_HOME/config.toml")"
assert_equals 0 "$(grep -F -c -- "add|$SERVICE|api-key" "$EMPTY_LOG" || true)"
assert_not_file "$EMPTY_HOME/custom-subagents/state.json"

# An existing item is never overwritten or deleted when lifecycle work fails.
set +e
( CUSTOM_SUBAGENT_FAIL_AFTER_WRITE=1
  run_configure https://api.deepseek.com deepseek-chat \
    >"$CAPTURED/existing-failure.out" 2>"$CAPTURED/existing-failure.err" )
...
# A fresh Keychain item is removed if a later lifecycle write fails.
...
sh "$CONFIGURE" --model deepseek-reasoner >"$CAPTURED/failing-configure.out" 2>"$CAPTURED/failing-configure.err"
failure_status=$?
set -e
[ "$failure_status" -ne 0 ] || fail 'injected lifecycle failure unexpectedly succeeded'
if grep -F -x -- "$SERVICE|api-key" "$FAIL_STATE" >/dev/null 2>&1; then
  fail 'fresh Keychain item remained after lifecycle failure'
fi
assert_contains "$FAIL_LOG" "delete|$SERVICE|api-key"

# A failed rollback deletion remains visible and gives a non-secret recovery instruction.
...
CUSTOM_SUBAGENT_OSASCRIPT_BIN="$TEST_HELPERS/fake-osascript.sh" \
FAKE_SECURITY_STATE="$DELETE_FAIL_STATE" FAKE_SECURITY_LOG="$DELETE_FAIL_LOG" FAKE_DIALOG_SCRIPT_LOG="$FAKE_DIALOG_SCRIPT_LOG" \
FAKE_DIALOG_VALUE="$SENTINEL" FAKE_SECURITY_FAIL=delete-generic-password CUSTOM_SUBAGENT_FAIL_AFTER_WRITE=1 \
sh "$CONFIGURE" --model deepseek-reasoner >"$CAPTURED/delete-fail.out" 2>"$CAPTURED/delete-fail.err"
delete_fail_status=$?
set -e
...
# A delete failure after lifecycle cleanup can be retried by a second uninstall.
FAKE_DIALOG_VALUE=retry-secret
run_configure https://api.deepseek.com deepseek-chat >"$CAPTURED/reinstall-before-retry.out" 2>"$CAPTURED/reinstall-before-retry.err"
set +e
...
if grep -F -x -- "$SERVICE|api-key" "$FAKE_STATE" >/dev/null 2>&1; then
  fail 'retry uninstall left the DeepSeek Keychain item behind'
fi
