...
for plugin_name in $PLUGIN_NAMES; do
  ...
  for shared_name in keychain.sh lifecycle.sh operation-lock.sh prompt-secret.js state.js store-keychain.exp model-discovery.sh model-discovery.js choose-model.js; do
